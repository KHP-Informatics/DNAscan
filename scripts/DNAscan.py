...
import argparse , sys , string , os , os.path , re ,  paths , subprocess
...
parser = argparse.ArgumentParser(prog='python DNAscan.py', usage='%(prog)s [options] -format "string" -reference "string" -in "string"', description = '############DNAscan help Message############ \n\nDNAscan uses the file paths.py to locate the needed tools and files. Please make sure your paths.py file is properly filled \n\nUsage example: \n\npython rDNAscan.py -format fastq -out /home/user/test/ -in sample_sorted_1.fq.gz -reference hg19 -alignment -variantcalling -results_report\n\nPlease check the following list of optional and required options\n\n################################################', formatter_class=RawTextHelpFormatter)

parser.add_argument('-mode' , required=True , action = "store" , dest = "mode" , default = "fast" , help = 'options are fast, normal, intensive [string] (default = "fast")' )

parser.add_argument( '-filter_string' , action = "store" , dest = "filter_string" , default = "" , help = 'bcftools filter string, eg GQ>20 & DP>10 (Default = "")'  )

parser.add_argument( '-paired', action = "store" , dest = "paired" , default = "1" , help = 'options are 1 for paired end reads and 0 for single end reads (Default = "1")' )

parser.add_argument( '-vcf' , action = "store" , dest = "vcf" , help = 'complementary vcf file' )

parser.add_argument( '-in2' , action = "store" , dest = "input_file2" , help = 'input file 2, for paired end reads only (usually fastq file)' )

parser.add_argument( '-iobio' , action = "store_true" , dest = "iobio" , help = 'if this flag is set iobio services will be started at the end of the analysis (Default = "False")' , default = False ) 
...
parser.add_argument( '-BED' , action = "store_true" , dest = "BED" , help = 'restrict the analysis to the regions in the bed file (Default = "False") ' , default = False ) 

parser.add_argument( '-virus' , action = "store_true" , dest = "virus" , help = 'if this flag is set DNAscan will perform viral scanning (Default = "False")  ' , default = False ) 

parser.add_argument( '-bacteria' , action = "store_true" , dest = "bacteria" , help = 'if this flag is set DNAscan will perform bacteria scanning (Default = "False") ' , default = False ) 

parser.add_argument( '-custom_microbes' , action = "store_true" , dest = "custom_microbes" , help = 'if this flag is set DNAscan will perform a customized microbe scanning according to the provided microbe data base in paths.py (Default = "False")  ' , default = False ) 

parser.add_argument( '-variantcalling', action = "store_true" , dest = "variantcalling" , help = 'if this flag is set DNAscan will perform snv and indel calling (Default = "False")  ' , default = False ) 

parser.add_argument( '-annotation', action = "store_true" , dest = "annotation" , help = 'if this flag is set DNAscan will annotate the found variants (Default = "False")  ' , default = False ) 

parser.add_argument( '-results_report', action = "store_true" , dest = "results_report" , help = 'if this flag is set DNAscan generate a results report (Default = "False")  ' , default = False ) 

parser.add_argument( '-alignment_report', action = "store_true" , dest = "alignment_report" , help = 'if this flag is set DNAscan generate an alignment report (Default = "False") ', default = False ) 

parser.add_argument( '-sequencing_report', action = "store_true" , dest = "sequencing_report" , help = 'if this flag is set DNAscan generate a report describing the input sequencing data (Default = "False") ' , default = False ) 

parser.add_argument( '-calls_report', action = "store_true" , dest = "calls_report" , help = 'if this flag is set DNAscan generate a report describing the found snvs and indels (Default = "False")' , default = False ) 

parser.add_argument( '-sample_name' , action = "store" , dest = "sample_name" , default = "sample" , help = 'specify sample name [string] (default = "sample")' )

parser.add_argument( '-rm_dup', action = "store_true" , dest = "rm_dup" , help = 'if this flag is set DNAscan will remove duplicates from the sample bam file (Default = "False")' , default = False ) 

parser.add_argument( '-debug' , action = "store_true" , dest = "debug" , help = 'if this flag is set DNAscan will not delete intermediete and temporary files (Default = "False")' , default = False ) 
...
requiredNamed.add_argument( '-reference' , required=True , action = "store" , dest = "reference" , help = 'options are hg19, hg38 [string]' )

requiredNamed.add_argument( '-in' , required=True , action = "store" , dest = "input_file" , help = 'input file [string]' )
...
if BED == True :
    ...
    #splitting the analysis region into subsents of equal length to distribute the work across the available threads.
...
if alignment == True :        
    ...
    if format == "fastq" and "alignment.log" not in os.listdir( out + "logs" ) : 
        ...
        if paired == "1" :
            ...
            #7.1.2 Normal and intensive modes use HISAT2 to align all reads, then soft-clipped and unaligned reads are realigned with BWA mem
                
            if mode == "normal" or mode == "intensive" :
                
                
                #Intensive mode uses GATK haplotype caller which does not support read group missing anymore. Default values for RG id, lb, pl, pu, and sm are defined in paths.py. Please change them as needed.
                ...
                if RG == True :
                    ...
                    rg_option_bwa = " -R '@RG\tID:%s\tLB:%s\tPL:%s\tRGPU:%s\tSM:%s' " %( RG_ID , RG_LB , RG_PL , RG_PU , RG_SM)
                    
                else :
                    
                    rg_option = ""

                os.system( "%shisat2 %s  --no-softclip --no-spliced-alignment -p %s -x %s -1 %s -2 %s | %ssamtools view -Sb -  | %ssamtools sort -T %s -o %ssorted.bam ; %ssamtools index %ssorted.bam" %( path_hisat , rg_option_hisat2 , num_cpu , path_hisat_index , input_file , input_file2 , path_samtools , path_samtools , out , out , path_samtools , out ) )

                os.system( "%ssamtools view -@ %s -bhf 4 %ssorted.bam | samtools bam2fq - > %sunaligned_reads.fq" %( path_samtools , num_cpu , out , out ) )

                os.system( "%sbwa mem %s -t %s %s %sunaligned_reads.fq | %ssamtools view -Sb -  | %ssamtools sort -T %s -o %ssorted_bwa.bam ; %ssamtools index %ssorted_bwa.bam " %( path_bwa , rg_option_bwa , num_cpu , path_bwa_index , out , path_samtools , path_samtools , out , out , path_samtools , out ) )
        ...
        if paired == "0" :
            ...
            #7.2.2 Normal and intensive modes use HISAT2 to align all reads, then soft-clipped and unaligned reads are realigned with BWA mem
            ...
            if mode == "normal" or mode == "intensive" :  
                ...
                os.system( "%shisat2  --no-softclip --no-spliced-alignment -p %s -x %s -U %s | %ssamtools view -Sb -  | %ssamtools sort -T %s -o %ssorted.bam ; %ssamtools index %ssorted.bam" %( path_hisat , rg_option_hisat2 , num_cpu , path_hisat_index , input_file , path_samtools , path_samtools , out , out , path_samtools , out ) )
...
if variantcalling == True:
    ...
    if vcf :
        ...
        if "VC.log" in os.listdir( out + "logs" ) :
            ...
            #10.1 GATK hc indel calling (only performed in intensive mode)
            #In intesive mode, DNAscan calls snvs with Freebayes and indels with GATK hc. GATK hc is used only on those positions of the genome for which the alignment stage identifies one insertion or deletion in at least on read. 
...
#12. Perform known expansions search with ExpansionHunter
#Known expansions have to be described in a json file placed in the json folder (paths.py).  
...
if annotation == True :
    ...
    if "annovar.log" in os.listdir( out + "logs" ) : 
        ...
        # If intensive mode variant calling was performed, DNAscan called snvs with Freebayes and indels with GATK hc, resulting in two vcf files. For the annotation step these two files are merged together.
        ...
        os.system( "%stable_annovar.pl  --thread %s --vcfinput %s%s_sorted.vcf.gz %s -buildver %s -remove -protocol refGene,dbnsfp30a,clinvar_20170130,avsnp147,cadd -operation g,f,f,f,f -nastring . --outfile %s/annovar.vcf" %( path_annovar , num_cpu , out , sample_name , path_annovar_db , reference , out ) )
...
if virus == True or bacteria == True  or custom_microbes == True:
    ...
    if "microbes.log" in os.listdir( out + "logs" ) : 
        ...
        if custom_microbes == True :
                ...
                os.system( "%shisat2 --no-spliced-alignment -p %s -x %s -U %suniligned_reads.fastq.gz | %ssamtools view -hSb -  | %ssamtools sort -T %stemp.file -o %soutput_custom_microbes.bam -" %( path_hisat , num_cpu , path_custom_microbes_index , out , path_samtools , path_samtools , out , out ) )

                os.system( "%ssamtools index %soutput_custom_microbes.bam; %ssamtools idxstats %soutput_custom_microbes.bam > %scustom_microbes_stats.txt" %( path_samtools , out , path_samtools , out , out ) )

                #15.2.6 Generates user-selected microbes report
                
                os.system( "awk \'{print $1}\' %scustom_microbes_stats.txt > %scustom_microbes_list.txt ; for i in $(cat %scustom_microbes_list.txt| grep ref); do printf \"$i \"; %ssamtools depth %soutput_custom_microbes.bam -r $i | awk \'$3>0 {print $0}\' | wc -l; done > %scustom_microbes_coverage_stats.txt" %( out , out , out , path_samtools , out , out ) )

                custom_microbes_coverage_stats = open( '%scustom_microbes_coverage_stats.txt' %( out ) , 'r' )

                custom_microbes_coverage_stats_lines = custom_microbes_coverage_stats.readlines()
            
                custom_microbes_stats_file = open( '%scustom_microbes_stats.txt' %( out ) , 'r' )

                custom_microbes_stats_file_lines = custom_microbes_stats_file.readlines()
            
                i = 0
            
                custom_microbes_results = open('%sresults/custom_microbes_results.txt' %( out ) , 'w' )
            
                custom_microbes_results.write( "Id\tGenome_lenth\tNumber_of_reds\tCoverage\n" )
            
                while i < len(custom_microbes_coverage_stats_lines) : 
                
                    custom_microbes_results.write( "%s\t%s\t%s\t%s\n" %( custom_microbes_stats_file_lines[i] , custom_microbes_stats_file_lines[i].split('\t')[1] , custom_microbes_stats_file_lines[i].split('\t')[2] , custom_microbes_coverage_stats_lines[i].split(' ')[1].strip() ) )
                ...
                if debug == False :
                    ...
                    os.system( "rm %scustom_microbes_stats.txt  %scustom_microbes_coverage_stats.txt" %( out , out  ) )
