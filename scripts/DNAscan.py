...
parser.add_argument(
    ...
    dest="alsgenescanner",
    default=False,
    help='if this flag is set DNAscan will perform the ALSgeneScanner analyses and reports (Default = "False")')
...
parser.add_argument(
    ...
    action="store",
    dest="rm_dup",
    help='do you want to mark duplicates while aligning the reads to the reference genome? Options are True or False [string] (Default = "True")',
    default="True")
...
parser.add_argument(
    ...
    action="store_true",
    dest="exome",
    help='if this flag is set DNAscan will only look for variants in the whole exome (Default = "False")',
    default=False)
...
if BED or path_gene_list:
    ...
    # distribute the work across the available threads.
        ...
        os.system("split -d -l `wc -l %stmp/tmp.bed | awk '{if ($1/%s > int($1/%s)) print int($1/%s)+1; else print int($1/%s)}'` %stmp/tmp.bed %stmp/" %(out, num_cpu, num_cpu,num_cpu,num_cpu, out, out ))
        ...
        if path_gene_list :
            ...
            os.system("zgrep -iwf %s %s%s_gene_names.txt.gz | awk '{print $1}' > %smatched_genes_codes.txt" %(path_gene_list,path_to_db,reference,out))
            ...
            os.system("%sbedtools merge -i  %scustom_sorted.bed> %scustom.bed" %(path_bedtools, out , out))

            os.system("rm %scustom_sorted.bed %scustom_tmp.bed" %(out, out))

            os.system("awk \'{i=$2; while (i < $3) {print $1\"\t\"i\"\t\"i+1 ;  i++}}\' %scustom.bed > %stmp/tmp.bed" %(out, out))
...
if filter_string:
    ...
    variant_results_file = "%sresults/%s_sorted_filtered.vcf.gz" %(out, sample_name)
...
if virus or bacteria or custom_microbes:
    ...
    if "microbes.log" in os.listdir(out + "logs"):
        ...
        os.system(
            ...
            (path_samtools,num_cpu, bam_file, path_samtools,out, num_cpu,out,out,out,out))
        ...
        if virus:
            ...
            os.system(
                ...
                (path_hisat, num_cpu, path_virus_index, out, path_samtools,num_cpu, path_samtools,num_cpu, out, out))
...
if alignment_report:
    ...
    if "alignment_report.log" in os.listdir(out + "logs"):
        ...
        os.system(
            "%ssamtools flagstat -@ %s %s > %sreports/%s_flagstat.txt" %
...
if sequencing_report:
    ...
    if "sequencing_report.log" in os.listdir(out + "logs"):
        ...
        os.system("%sfastqc %s -o %sreports -f %s -t %s %s %s" %
                  (path_fastqc, java_option, out, format, num_cpu, input_file, input_file2))
...
if alignment_report or calls_report or sequencing_report:
    ...
    if "multiqc.log" in os.listdir(out + "logs"):
        ...
        os.system("%smultiqc -o %sreports %sreports" % (path_multiqc, out, out))
             ...
             for j in vcf:
                ...
                if check and check1:
                    ...
                    if counter == 0 :
                      ...
                      replaced_1 = re.sub('=[a-z,A-Z,0-9,\.,\_,\-,:,>,<]+', '', infos)
                    ...
                    replaced = re.sub('[a-z,A-Z,0-9,\.,\_,\-,:,>,<]+=', '', infos)

                    out_file_all.write(
                        '%s\t%s\t%s\t%s\t%s\t%s\n' %
                        (j.split('\t')[0],
                            j.split('\t')[1],j.split('\t')[3],
                            j.split('\t')[4], j.split('\t')[-1].split(':')[0],replaced))
...
if alsgenescanner:
    ...
    os.system("python3 %s/alsgenescanner.py %s/annovar.vcf.hg19_multianno.txt %s/results/%s_alsgenescanner_all.txt" %(path_scripts, out, out, sample_name))
    os.system("cat %s/results/%s_alsgenescanner_all.txt | head -1 > %s/results/%s_alsgenescanner_alsod.txt; cat %s/results/%s_alsgenescanner_all.txt | grep -iwf %s/list_genes_alsod.txt >> %s/results/%s_alsgenescanner_alsod.txt" %(out,sample_name,out,sample_name, out,sample_name, path_to_db, out,sample_name))
    os.system("cat %s/results/%s_alsgenescanner_all.txt | head -1 > %s/results/%s_alsgenescanner_clinvar.txt ; cat %s/results/%s_alsgenescanner_all.txt | grep -iwf %s/list_genes_clinvar.txt >> %s/results/%s_alsgenescanner_clinvar.txt" %(out,sample_name,out,sample_name, out,sample_name, path_to_db, out,sample_name))
    os.system("cat %s/results/%s_alsgenescanner_all.txt | head -1 > %s/results/%s_alsgenescanner_manual_review.txt ; cat %s/results/%s_alsgenescanner_all.txt | grep -iwf %s/list_genes_manual_review.txt >> %s/results/%s_alsgenescanner_manual_review.txt" %(out,sample_name,out,sample_name, out, sample_name, path_to_db, out,sample_name))
