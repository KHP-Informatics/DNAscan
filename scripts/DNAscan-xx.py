...
if variantcalling:
    ...
    if vcf:
        ...
        if "VC.log" in os.listdir(out + "logs"):
            ...
            variant_results_file = "%sresults/%s_sorted.vcf.gz" % (
                out, sample_name)
            ...
            if "VC_freebayes.log" not in os.listdir(out + "logs"):
                ...
                if mode == "intensive":
                    ...
                    # For the annotation step these two files are merged
                    # together.
                    ...
                    os.system(
                        ...
                        (path_java, path_gatk, path_reference, out, out, out, sample_name))

                    os.system(
                        "perl %svcf-sort.pl  %s%s.vcf | bgzip -c > %s%s_sorted.vcf.gz" %
                        (path_scripts, out, sample_name, out, sample_name))

                    variant_results_file = "%s%s_sorted.vcf.gz" % (
                        out, sample_name)
...
if annotation:
    ...
    if "annovar.log" in os.listdir(out + "logs"):
        ...
        variant_results_file = "%sresults/%s_annotated.vcf.gz" % (
            out, sample_name)
...
if results_report:
    ...
    if "annovar.log" not in os.listdir(out + "logs"):
        ...
        if "results_report.log" in os.listdir(out + "logs"):
            ...
            out_file_all = open(
                '%sreports/output_all_variants.txt' %
                (out), 'w')
